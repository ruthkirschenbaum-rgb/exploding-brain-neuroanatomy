...
## Neuroanatomy · Exploded adaptation

This project retains 161 source components from Human Atlas's BodyParts3D export. It repacks their original position, normal, and index buffers into a single CNS model and applies course-specific display groups and colors. The 31 spinal cord segment objects are separately authored schematic geometry extending caudally from the source medulla. They are not original BodyParts3D tissue meshes. Lecture slides are user-provided course material and have separate rights.
